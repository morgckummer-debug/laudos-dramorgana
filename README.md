...
- **Espaçamentos e tamanhos do laudo em `em`, nunca em px/pt fixos.** Antes de imprimir, `autoFitPages()` tenta caber o laudo numa folha só apertando a entrelinha (1,15 → 1,0) e, se ainda não couber, reduzindo a fonte (12pt → 11pt, mínimo). Isso só funciona de verdade se **tudo** encolher junto: com margens em px fixos, o texto encolhia mas o espaço entre parágrafos não, e o passo 12pt→11pt rendia só ~3,5% de altura em vez de ~8,3%. Por isso as margens dos parágrafos, o quadro de identificação, o título, os títulos de seção, o rodapé e a assinatura estão todos em `em` (calibrados para render exatamente o mesmo na fonte padrão de 12pt). Ao mexer no CSS de `.paper`, mantenha `em` — um px fixo ali volta a travar o auto-fit.
- **A lista de médicos (`.combo-list`) precisa ficar escondida na impressão.** Ela é movida para o `<body>` para não ser cortada pelo `overflow:hidden` do card, então não é descendente de `#formCol` e não some com a regra que esconde o formulário; sem uma regra própria em `@media print`, imprimir com a lista aberta joga o dropdown por cima do laudo.
