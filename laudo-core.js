...
function criarMotorLaudo(cfg){
  ...
  function draftFingerprint(s){
    ...
    // só existe na tireoide (lista de nódulos); `s.linfoDUids`/`s.linfoEUids`
    // só existem no cervical-doppler (linfonodos à direita/esquerda); nos
    // laudos que não têm cada uma dessas chaves elas são sempre undefined dos
    // dois lados da comparação, então não mudam nada para eles.
    return JSON.stringify([s.fields, s.uids, s.miomaUids, s.noduloUids, s.linfoDUids, s.linfoEUids, s.checked, s.overrides, s.paper, s.anexoFmf]);
  }
